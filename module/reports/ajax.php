    ...
    if(!empty($requestData["search"]["value"]) or !empty($requestData["columns"][1]['search']['value']) or !empty($requestData["columns"][2]['search']['value']) or !empty($requestData["columns"][3]['search']['value']) or !empty($requestData["columns"][4]['search']['value']) ) {  // get data with search
        ...
        $getData = easySelectA(array(
            ...
            "fields"    => "
                            ...
                            if(child_product.product_parent_id is not null, 1, 0) as hasChildProduct,
                            round(coalesce(variable_initial_qty, initial_qty, 0), 2) as initial_qty,
                            round(coalesce(variable_production_qty, production_qty, 0), 2) as production_qty,
                            round(coalesce(variable_sale_qty, sale_qty, 0), 2) as sale_qty,
                            round(coalesce(variable_wastage_sale_qty, wastage_sale_qty, 0), 2) as wastage_sale_qty,
                            round(coalesce(variable_sale_return_qty, sale_return_qty, 0), 2) as sale_return_qty,
                            round(coalesce(variable_purchase_qty, purchase_qty, 0), 2) as purchase_qty,
                            round(coalesce(variable_purchase_order_qty, purchase_order_qty, 0), 2) as purchase_order_qty,
                            round(coalesce(variable_purchase_return_qty, purchase_return_qty, 0), 2) as purchase_return_qty,
                            round(coalesce(variable_transfer_in_qty, transfer_in_qty, 0), 2) as transfer_in_qty,
                            round(coalesce(variable_transfer_out_qty, transfer_out_qty, 0), 2) as transfer_out_qty,
                            round(coalesce(variable_specimen_copy_qty, specimen_copy_qty, 0), 2) as specimen_copy_qty,
                            round(coalesce(variable_specimen_copy_return_qty, specimen_copy_return_qty, 0), 2) as specimen_copy_return_qty,
                            round(coalesce(variable_expired_qty, expired_qty, 0), 2) as expired_qty,
            ...
            "join"      => array(
                ...
                "left join (
                    ...
                    SELECT
                        ...
                        sum(purchase_qty) as variable_purchase_qty,
                        sum(purchase_item_subtotal) as variable_purchase_item_subtotal,
                        sum(purchase_order_qty) as variable_purchase_order_qty,
                        sum(purchase_return_qty) as variable_purchase_return_qty,
                        sum(transfer_in_qty) as variable_transfer_in_qty,
                        sum(transfer_out_qty) as variable_transfer_out_qty,
                        sum(specimen_copy_qty) as variable_specimen_copy_qty,
                    ...
                    left join (
                        ...
                        where warehouse $warehouse_filter
                        group by vp_id
                    ) as base_stock on base_stock.vp_id = childProduct.product_id

                    where childProduct.is_trash = 0
    ...
    <div class="modal-body">
        ...
        <div class="no-print">
            <div style="display: block; width: 200px; margin: auto;" class="form-group">
                <a onClick='BMS.MAIN.printPage(this.href, event);' class="btn btn-primary" href="<?php echo full_website_address(); ?>/invoice-print/?autoPrint=true&invoiceType=posSale&id=<?php echo $_GET["id"]; ?>"> <i class="fa fa-print"></i> Print</a>
                <a target="_blank" class="btn btn-primary" href="<?php echo full_website_address(); ?>/invoice-print/?invoiceType=posSale&id=<?php echo $_GET["id"]; ?>"> <i class="fa fa-eye"></i> View Invoice</a>
            </div>
        </div>
...
if(isset($_GET['page']) and $_GET['page'] == "showReturnProducts") {
    ...
    <div class="modal-body">
        ...
        <div class="no-print">
            <div style="display: block; width: 200px; margin: auto;" class="form-group">
                <a target="_blank" class="btn btn-primary" href="<?php echo full_website_address(); ?>/invoice-print/?autoPrint=true&invoiceType=produtReturn&id=<?php echo $_GET["id"]; ?>"> <i class="fa fa-print"></i> Print</a>
            </div>
        </div>
