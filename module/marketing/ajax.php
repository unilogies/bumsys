...
if (isset($_GET['page']) and $_GET['page'] == "specimenCopyOverview") {
    ...
    if (  !empty($requestData["search"]["value"]) or 
        ...
        $productEditionFilter = !empty($requestData["columns"][3]['search']['value']) ? " AND product_edition = '{$requestData["columns"][3]['search']['value']}' " : "";

        $dateFilter = "";
        $dateFilterForDistribution = "";
        if(!empty($requestData["columns"][2]['search']['value'])) {
            $dateRange = explode(" - ", safe_input($requestData["columns"][2]['search']['value']));
            $dateFilter = " AND stock_entry_date between '{$dateRange[0]}' AND '{$dateRange[1]}' ";
            $dateFilterForDistribution = " AND scd_date between '{$dateRange[0]}' AND '{$dateRange[1]}' ";
        }
        ...
        $getData = easySelectA(array(
            ...
            "join"    => array(
                ...
                where is_trash = 0 {$dateFilterForDistribution}
            ...
            "where" => array(
                "scMain.is_trash = 0 and scMain.stock_sc_id is not null {$productEditionFilter} {$dateFilter}",
                " AND product_name LIKE" => "%" . $requestData['search']['value'] . "%",
                " AND stock_employee_id" => $requestData["columns"][1]['search']['value'],
    ...
    if (isset($getData['count']) and $getData['count'] > 0) {
        ...
        foreach ($getData['data'] as $key => $value) {
            ...
            $allNestedData[] = $value['scDispatch'] * $value['product_sale_price'];
            $allNestedData[] = $value['getTotalDistributed'] * $value['product_sale_price'];
